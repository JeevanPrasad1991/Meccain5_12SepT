...
public class CheckOutStoreActivity extends Activity implements LocationListener {
    ...
    ArrayList<CoverageBean> coverageBeenList = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        db.open();
        coverageBeenList = db.getCoverageSpecificData(store_id);
        ...
        protected String doInBackground(Void... params) {
            ...
            try {
                ...
                String onXML = "[STORE_CHECK_OUT_STATUS][USER_ID]"
                        ...
                        + store_id
                        + "[/STORE_ID][LATITUDE]"
                        + coverageBeenList.get(0).getLatitude()
                        + "[/LATITUDE][LOGITUDE]"
                        + coverageBeenList.get(0).getLongitude()
                        + "[/LOGITUDE][CHECKOUT_DATE]"
                        + coverageBeenList.get(0).getVisitDate()
                        + "[/CHECKOUT_DATE][CHECK_OUTTIME]"
                        + getCurrentTime()
                        + "[/CHECK_OUTTIME][CHECK_INTIME]"
                        + coverageBeenList.get(0).getInTime()
