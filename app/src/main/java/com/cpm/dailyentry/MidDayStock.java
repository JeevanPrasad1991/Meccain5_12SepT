...
public class MidDayStock extends Activity implements OnClickListener{
			...
			public boolean onChildClick(ExpandableListView parent, View v,
				...
				Toast.makeText(
						...
						listDataHeader.get(groupPosition).getBrand()
								...
								childPosition).getSku(), Toast.LENGTH_SHORT).show();
	...
	private void prepareListData() {
		...
		if(brandData.size()>0){
			...
			for(int i=0;i<brandData.size();i++){
				...
				if(!(skuData.size()>0) || (skuData.get(0).getMidday_stock()==null) || (skuData.get(0).getMidday_stock().equals(""))){
	...
	boolean validateData(
		...
		for (int i = 0; i < listDataHeader2.size(); i++) {
			for (int j = 0; j < listDataChild2.get(listDataHeader.get(i))
					.size(); j++) {
				String miday_stock = listDataChild.get(listDataHeader.get(i)).get(j).getMidday_stock();
	...
	public class ExpandableListAdapter extends BaseExpandableListAdapter {
		...
		public View getChildView(final int groupPosition, final int childPosition,
				...
				public void onFocusChange(View v, boolean hasFocus) {
					...
					if (!hasFocus) {
						...
						if (value1.equals("")) {
							...
							_listDataChild.get(listDataHeader.get(groupPosition)).get(childPosition).setMidday_stock("");

						} else {
							ischangedflag=true;

							_listDataChild.get(listDataHeader.get(groupPosition)).get(childPosition).setMidday_stock(value1);
			...
			holder.etmidstock.setText(childText.getMidday_stock());
	...
	private void showKeyboardWithAnimation() {
		if (mKeyboardView.getVisibility() == View.GONE) {
			Animation animation = AnimationUtils.loadAnimation(MidDayStock.this, R.anim.slide_in_bottom);
